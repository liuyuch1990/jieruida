...
Page({
  data: {
    ...
    activeTab: 0,
    url: encodeURIComponent('https://www.jieruida-qd.com/dist/index.html#/3pageEdit?id='),
    refund_type: 0,
    refundloc_orderid: '',
  ...
  getListall() {
    ...
    util.request(api.OrderByUserAndActivity, {
      mobile: userinfo.mobile,
      orderType: this.options.index
  ...
  payMyOrder: function (event) {
    let that = this;
    let id = event.currentTarget.id;
    pay.payOrder(id).then(res => {
      this.onLoad()
    }).catch(res => {
      this.onLoad()
    });
    ...
    wx.showModal({
      ...
      success: function (sm) {
        if (sm.confirm) {
          wx.request({
            //项目的真正接口，通过字符串拼接方式实现
            url: "https://www.jieruida-qd.com/bargin/delete",
            header: {
    ...
    wx.showModal({
      ...
      success: function (sm) {
        if (sm.confirm) {
          wx.request({
            //项目的真正接口，通过字符串拼接方式实现
            url: "https://www.jieruida-qd.com/bargin/copy",
            header: {
              "content-type": "application/json;charset=UTF-8"
            },
