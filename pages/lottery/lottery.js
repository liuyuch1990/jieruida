...
Page({
  data: {
    ...
    activeTab: 0,
    url: encodeURIComponent('https://www.jieruida-qd.com/dist/index.html/#/eggEdit?id='),
    refund_type: 0,
    refundloc_orderid: '',
    ...
    wx.showModal({
      ...
      success: function (sm) {
        if (sm.confirm) {
          wx.request({
            //项目的真正接口，通过字符串拼接方式实现
            url: "https://www.jieruida-qd.com/lottery/delete",
            header: {
    ...
    wx.showModal({
      ...
      success: function (sm) {
        if (sm.confirm) {
          wx.request({
            //项目的真正接口，通过字符串拼接方式实现
            url: "https://www.jieruida-qd.com/lottery/copy",
            header: {
              "content-type": "application/json;charset=UTF-8"
            },
