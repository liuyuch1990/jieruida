...
Page({
  data: {
    ...
    activeTab: 0,
    url: encodeURIComponent('https://www.jieruida-qd.com/dist/index.html#/2pageEdit?id='),
    refund_type: 0,
    refundloc_orderid: '',
  ...
  viewOrder: function (event) {
    wx.navigateTo({
      url: '/pages/order/order?id=' + event.currentTarget.id +"&type=jizan", //
      success: function (res) {
  ...
  OutUrl(e) {
    wx.navigateTo({
      url: '/pages/out/out?id=' + e.currentTarget.id + '&url='+this.data.url + '&footer=#/2pageEdit', //
      success: function (res) {
        console.info("回退后的参数", res);
      },       //成功后的回调；
      fail: function (res) { },         //失败后的回调；
    ...
    wx.showModal({
      ...
      success: function (sm) {
        if (sm.confirm) {
          wx.request({
            //项目的真正接口，通过字符串拼接方式实现
            url: "https://www.jieruida-qd.com/gather/delete",
            header: {
    ...
    wx.showModal({
      ...
      success: function (sm) {
        if (sm.confirm) {
          wx.request({
            //项目的真正接口，通过字符串拼接方式实现
            url: "https://www.jieruida-qd.com/gather/copy",
            header: {
              "content-type": "application/json;charset=UTF-8"
            },
